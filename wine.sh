...
shift 2
$dir/install-packages-from-koji.sh p=$package,rpm=$package,v=$version,r=$release    rpm=$package-{alsa,capi,cms,core,ldap,openal,opencl,pulseaudio,twain}    a=i686,rpm=$package-{alsa,capi,cms,core,ldap,openal,opencl,pulseaudio,twain}    a=noarch,rpm=$package-{common,arial-fonts,courier-fonts,desktop,filesystem,fonts,fixedsys-fonts,marlett-fonts,ms-sans-serif-fonts,small-fonts,symbol-fonts,system-fonts,systemd,tahoma-fonts,times-new-roman-fonts,webdings-fonts,wingdings-fonts}    "$@"
